...
interface ArcTimelineProps extends ComponentPropsWithoutRef<"div"> {
  ...
  autoplay?: {
    /**
     * Enable autoplay, default is false
     */
    enabled?: boolean;
    /**
     * Delay between steps in milliseconds, default is 3000 (3 seconds)
     */
    delay?: number;
    /**
     * Whether to pause autoplay on hover, default is true
     */
...
export function ArcTimeline(props: ArcTimelineProps) {
  ...
  // Track current step for autoplay
  ...
  }, [autoplayEnabled, autoplayDelay, isHovered, pauseOnHover, totalSteps]);

  // Update rotation when current step changes
  useEffect(() => {
    const angle =
      currentStepIndex *
          ...
          return (
            <div key={`${lineIndex}`}>
              {line.steps.map((step, stepIndex) => {
                ...
                // Calculate global step index for this step
